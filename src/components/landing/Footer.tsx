    ...
    <footer className="border-t border-border bg-muted">
      <div className="container-hz py-12 md:py-16">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1fr_1fr]">
          <div>
            <img src="/main_logo.png" alt="Safe Flow" className="h-10 w-auto object-contain" />
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-muted-foreground">Digital Investigator for Financial Crime Intelligence</p>
          </div>
          {[
            ...
            <div key={heading as string}>
              <Mono className="text-muted-foreground">{heading as string}</Mono>
              <ul className="mt-4 space-y-2 text-sm text-foreground/80">
                {(links as { label: string; href: string }[]).map((item) => <li key={item.label}><a href={item.href} className="transition-colors hover:text-foreground">{item.label}</a></li>)}
              </ul>
            </div>
          ))}
        </div>
