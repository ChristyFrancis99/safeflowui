...
import {
  Briefcase, Microscope, AlertTriangle, Network, FileCheck, FileText, Eye,
  Settings, LogOut, Menu, X, CheckSquare, TrendingUp, ClipboardList,
  Users, Shield, Sliders, Lock, History,
} from "lucide-react";
...
export function Sidebar({ open = true, onOpenChange, userRole: userRoleProp }: SidebarProps) {
  ...
  const handleNavClick = (href: string) => navigate({ to: href });
  const handleSignOut = () => { clearRole(); navigate({ to: "/sign-in" }); };
  const roleLabelMap: Record<RoleId, string> = {
    investigator: "Investigator View", manager: "Manager View", administrator: "Administrator View",
  };
  ...
  return (
    <>
      <button
        ...
        className="fixed left-4 top-4 z-50 rounded-md border border-border bg-background p-2.5 lg:hidden"
      ...
      <aside className={cn(
        "fixed left-0 top-0 z-40 h-screen w-64 bg-card transition-transform duration-200 ease-out lg:relative lg:translate-x-0",
        isOpen ? "translate-x-0" : "-translate-x-full",
      )}>
        <div className="flex h-full flex-col overflow-y-auto border-r border-border">
          <button
            onClick={() => navigate({ to: "/dashboard" })}
            className="border-b border-border p-5 text-left transition-colors hover:bg-muted/30 focus-visible:outline-none"
            aria-label="Go to Dashboard"
          >
            <div className="flex items-center gap-3">
              <Logo size="md" variant="navy" rounded="md" showLabel={false} />
              <div>
                <p className="text-xs font-bold tracking-[0.16em] text-foreground">Safe Flow</p>
                <Mono className="mt-0.5 text-[10px] text-muted-foreground">{roleLabelMap[activeRole]}</Mono>
              </div>
            </div>
            <div className="mt-3.5 flex items-center justify-between rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs">
              <span className="text-[11px] font-semibold capitalize text-foreground">{activeRole}</span>
              <span className="rounded-sm border border-teal/20 bg-teal/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-teal">Active</span>
            </div>
          </button>

          <nav className="flex-1 px-3 py-5">
            <p className="mb-2.5 px-3 font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Workspace Navigation</p>
            <ul className="space-y-1">
              {mainNav.map((item) => {
                const isSelected = location.pathname === item.href || (item.href !== "/dashboard" && location.pathname.startsWith(item.href));
                return (
                  <li key={item.id}>
                    <button
                      onClick={() => handleNavClick(item.href)}
                      className={cn(
                        "group relative flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-xs font-medium transition-colors duration-150",
                        isSelected ? "border-l-2 border-primary bg-primary/10 pl-2.5 font-semibold text-primary" : "text-muted-foreground hover:bg-muted/60 hover:text-foreground",
                        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20",
                      )}
                    >
                      <span className={cn("flex size-4 items-center justify-center", isSelected ? "text-primary" : "text-muted-foreground group-hover:text-foreground")}>{item.icon}</span>
          ...
          <div className="border-t border-border px-3 py-4">
            <button
              ...
              className="group flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-xs font-medium text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20"
            >
