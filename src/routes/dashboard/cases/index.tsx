...
function CasesListPage() {
  ...
  const riskClass = (level: string) => level === "CRITICAL" ? "bg-red-500/20 text-red-300 border-red-400/50 shadow-[0_0_12px_rgba(239,68,68,0.18)]" : level === "HIGH" ? "bg-orange-500/20 text-orange-300 border-orange-400/50 shadow-[0_0_12px_rgba(249,115,22,0.18)]" : level === "MEDIUM" ? "bg-yellow-500/20 text-yellow-300 border-yellow-400/50 shadow-[0_0_12px_rgba(234,179,8,0.18)]" : "bg-emerald-500/20 text-emerald-300 border-emerald-400/50 shadow-[0_0_12px_rgba(34,197,94,0.18)]";
    ...
    <DashboardLayout title="Cases Directory">
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-muted/30 text-xs font-semibold uppercase tracking-wider text-muted-foreground"><th className="px-6 py-4 text-left">Case ID</th><th className="px-6 py-4 text-left">Status</th><th className="px-6 py-4 text-left">Risk Level</th><th className="px-6 py-4 text-left">Alert / Action</th><th className="px-6 py-4 text-left">AI Recommendation</th><th className="px-6 py-4 text-left">Action</th></tr></thead><tbody className="divide-y divide-border">
          {filteredCases.map((caseItem) => <tr key={caseItem.id} className="hover:bg-muted/20 transition-colors"><td className="px-6 py-4"><Link to="/dashboard/cases/$caseId" params={{ caseId: caseItem.id }} className="font-semibold text-violet hover:underline flex items-center gap-1">{caseItem.id}<ArrowUpRight className="size-3" /></Link><p className="text-xs text-muted-foreground mt-0.5">{caseItem.title}</p></td><td className="px-6 py-4"><span className="inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full bg-muted text-foreground">{(caseItem as { status?: string }).status ?? "OPEN"}</span></td><td className="px-6 py-4"><div className="flex items-center gap-2"><span className={cn("inline-flex min-w-[72px] items-center justify-center rounded-lg border px-2.5 py-1 text-xs font-bold tracking-wide", riskClass(caseItem.risk.level))}>{caseItem.risk.level}</span><span className="text-xs text-muted-foreground">{caseItem.risk.value}/100</span></div></td><td className="px-6 py-4"><span className="text-xs text-foreground">{caseItem.alert}</span></td><td className="px-6 py-4"><span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-violet/10 text-violet">{caseItem.recommendation}</span></td><td className="px-6 py-4"><Button asChild size="sm" variant="outline" className="text-xs gap-1"><Link to="/dashboard/cases/$caseId" params={{ caseId: caseItem.id }}>View Workspace<ArrowUpRight className="size-3" /></Link></Button></td></tr>)}
