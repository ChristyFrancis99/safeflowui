...
function SignInPage() {
  ...
  const handleSignUp = () => {
    setInfoNotice("Sign up is coming soon. Please contact your Safe Flow administrator for access.");
  };
        ...
        <div className="grid min-h-[calc(100vh-2rem)] lg:grid-cols-[1.08fr_0.92fr]">
          <Reveal>
            <div className="relative hidden h-full min-h-[calc(100vh-2rem)] overflow-hidden lg:block">
              <img src="/public/login.png" alt="Safe Flow Digital Investigator" className="absolute inset-0 h-full w-full object-cover" />
            </div>
          </Reveal>
          <Reveal delay={80}>
            <div className="flex flex-col justify-center bg-background px-4 py-8 sm:px-6 lg:px-10 xl:px-14">
              <div className="mx-auto w-full max-w-md">
                <div className="mb-6 flex items-center justify-between gap-3 lg:hidden">
                  ...
                  <Link to="/" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">Back to home</Link>
                </div>
                ...
                <form className="mt-8 space-y-5" onSubmit={handleSubmit} noValidate>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button type="button" onClick={handleForgotPassword} className="text-sm font-medium text-gov transition-colors hover:text-violet focus:outline-none">Forgot password?</button>
                    </div>
                    <div className="relative">
                      <input id="password" type="password" value={password} onChange={(event) => { setPassword(event.target.value); if (passwordError) setPasswordError(""); }} placeholder="Enter your password" className={`w-full rounded-xl border bg-background px-3.5 py-3 pr-10 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:ring-2 focus:ring-violet/20 ${passwordError ? "border-risk-high focus:border-risk-high" : "border-border focus:border-violet/50"}`} />
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <Button type="submit" className="w-full rounded-xl sm:flex-1" size="lg">
                      Sign In <ArrowRight className="size-4" aria-hidden />
                    </Button>
                    <Button type="button" variant="outline" className="w-full rounded-xl sm:flex-1" size="lg" onClick={handleSignUp}>
                      Sign Up
                    </Button>
                  </div>
                </form>
